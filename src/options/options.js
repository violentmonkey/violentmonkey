...
var scriptList = function() {
  ...
  var dragging = {};
  function getRelativePos(e, ancestor) {
    var offset = {
      x: e.offsetX,
      y: e.offsetY,
    };
    for (var ele = e.target; ele && ele != ancestor; ele = ele.offsetParent) {
  ...
  function dragstart(e) {
    ...
    dragging.offset = getRelativePos(e, e.currentTarget);
    node.style.width = node.offsetWidth + 'px';
    node.style.left = e.clientX - dragging.offset.x + 'px';
    node.style.top = e.clientY - dragging.offset.y + 'px';
  ...
  function mouseup(e) {
    ...
    dragging.scroll = 0;
    var offset = wrap.getBoundingClientRect();
    node.style.left = e.clientX - dragging.offset.x - offset.left + 'px';
    node.style.top = e.clientY - dragging.offset.y - offset.top + wrap.scrollTop + 'px';
