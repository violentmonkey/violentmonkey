...
const pad00 = num => `${num < 10 ? '0' : ''}${num}`;
const pad000 = num => `${num < 10 && '00' || num < 100 && '0' || ''}${num}`;
...
export const DATE_FMT = {
  ...
  MM: date => pad00(date.getMonth() + 1), // 01 02 ... 11 12
  ...
  DD: date => pad00(date.getDate()), // 01 02 ... 30 31
  DDD: getDayOfYear, // 1 2 ... 364 365
  DDDD: date => pad000(getDayOfYear(date)), // 001 002 ... 364 365
  ...
  ww: date => pad00(getWeekOfYear(date)), // 01 02 ... 52 53
  Y: getYear,
  YY: date => pad00(getYear(date) % 100),
  YYYY: date => `${getYear(date)}`.slice(-4),
  H: date => date.getHours(), // 0 1 ... 22 23
  HH: date => pad00(date.getHours()), // 00 01 ... 22 23
  m: date => date.getMinutes(), // 0 1 ... 58 59
  mm: date => pad00(date.getMinutes()), // 00 01 ... 58 59
  s: date => date.getSeconds(), // 0 1 ... 58 59
  ss: date => pad00(date.getSeconds()), // 00 01 ... 58 59
  ...
  ZZ: date => { // -0700 -0600 ... +0600 +0700
    ...
    return `${tz < 0 ? '-' : '+'}${pad00(Math.floor(tza / 60))}${pad00(Math.floor(tza % 60))}`;
  },
};
