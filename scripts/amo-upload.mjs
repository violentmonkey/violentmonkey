...
async function handleAddon() {
  ...
  const fileName = `feature-injector-${version}${beta ? 'b' : ''}.xpi`;
  const url = `https://github.com/rburgessCEStrategy/Injector/releases/download/v${version}/${fileName}`;
  ...
  const releaseUrl = `https://github.com/rburgessCEStrategy/Injector/releases/tag/v${version}`;
  await signAddon({
...
main().then(
  () => {
    notifyReleaseStatus({
      ...
      description: `See the changelog at https://github.com/rburgessCEStrategy/Injector/releases/tag/v${process.env.VERSION}.`,
    });
  },
  (err) => {
