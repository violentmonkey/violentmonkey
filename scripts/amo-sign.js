...
async function main() {
  ...
  const result = await signAddon({
    xpiPath: beta
    ? path.join(process.env.TEMP_DIR, process.env.ASSET_SELF_HOSTED_ZIP)
    : path.join(process.env.ASSETS_DIR, process.env.ASSETS_ZIP),
    version,
    apiKey: process.env.AMO_KEY,
    apiSecret: process.env.AMO_SECRET,
    channel: beta ? 'unlisted' : 'listed',
    // should not be able to download if channel is listed
    downloadDir: beta ? process.env.ASSETS_DIR : process.env.TEMP_DIR,
    id: manifest.browser_specific_settings.gecko.id,
  });
  if (!beta) {
    // if not beta, the command will always fail and need to check status on AMO
    return;
  }
  if (!result.success) {
